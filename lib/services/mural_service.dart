...
class MuralService {
  ...
  // Obtener todos los comentarios del mural (sin filtrar por pregunta)
  static Future<List<Comment>> getAllMuralComments({String? userId}) async {
    try {
      final authService = AuthService();
      ...
      print('URL de todos los comentarios: $url');
      ...
      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);
        final comments = data.map((json) => Comment.fromJson(json)).toList();
        print('Total comentarios encontrados: ${comments.length}');
        ...
        throw Exception(
            'Error al obtener todos los comentarios: ${response.statusCode}');
      }
    } catch (e) {
      print('Error cargando todos los comentarios: $e');
      return getSampleComments();
    }
  }
  ...
  static Future<MuralQuestion?> getActiveMuralQuestion() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Error al obtener la pregunta activa: ${response.statusCode}');
      }
    } catch (e) {
  ...
  static Future<Comment?> createMuralComment(
      String text, String userId, Map<String, String> authHeaders,
      {String? questionId,
      User? user,
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (data['aprobado'] == true) {
          ...
          throw Exception(
              'Comentario rechazado: ${data['motivo'] ?? 'Contenido inapropiado'}');
        }
      } else {
  ...
  static Future<Map<String, dynamic>> reactToComment(
      String commentId, String userId, String reactionType) async {
    try {
      final authService = AuthService();
      final headers = authService.getAuthHeaders();
      final response = await http.post(
        Uri.parse(
            '$baseUrl/api/mural/comentarios/$commentId/reactions?userId=$userId&type=$reactionType'),
        headers: headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData.containsKey('likeCount')) {
          responseData['likeCount'] = (responseData['likeCount'] as int? ?? 0)
              .clamp(0, double.infinity)
              .toInt();
        }
        if (responseData.containsKey('dislikeCount')) {
          responseData['dislikeCount'] =
              (responseData['dislikeCount'] as int? ?? 0)
                  .clamp(0, double.infinity)
                  .toInt();
        }
  ...
  static Future<List<String>> uploadCommentImages(List<File> images,
      String userId, String commentId, Map<String, String> authHeaders) async {
    try {
      ...
      for (int i = 0; i < images.length; i++) {
        ...
        final multipartFile = await http.MultipartFile.fromPath(
          'images', // Cambiar para coincidir con el backend que espera 'images'
          file.path,
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Error al subir imágenes: ${response.statusCode} - $responseBody');
      }
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> getCommentReactions(
  ...
  static List<MuralQuestion> getSampleQuestions() {
    return [
      MuralQuestion(
        id: '1',
        question:
            '¿Cuál es tu experiencia más memorable con la artesanía peruana?',
        description:
            'Comparte con la comunidad tus vivencias y aprendizajes sobre las artesanías tradicionales de nuestro país.',
        category: 'artesanía',
      ...
      MuralQuestion(
        ...
        description:
            'Cuéntanos sobre ese sitio histórico o cultural que te llama la atención y qué esperas encontrar allí.',
        category: 'turismo',
      ...
      MuralQuestion(
        id: '3',
        question:
            '¿Cómo crees que podemos preservar mejor nuestras tradiciones culturales?',
        description:
            'Reflexiona sobre las formas de mantener vivas nuestras costumbres y tradiciones para las futuras generaciones.',
        category: 'reflexión',
  ...
  static List<Comment> getSampleComments() {
    return [
      Comment(
        id: '1',
        text:
            '¡Excelente pregunta! Mi experiencia más memorable fue en Cusco, donde aprendí a tejer con lana de alpaca. Los artesanos son muy pacientes y generosos con su conocimiento.',
        userId: 'user1',
      ...
      Comment(
        id: '2',
        text:
            'Para mí fue en Ayacucho, viendo cómo se hacen los retablos. Es increíble la paciencia y el detalle que ponen en cada pieza.',
        userId: 'user2',
      ...
      Comment(
        id: '3',
        text:
            'Me encantaría visitar Chan Chan en Trujillo. La arquitectura de barro es fascinante y quiero aprender más sobre la cultura Chimú.',
        userId: 'user3',
        userName: 'Ana López',
        userProfileImage: null,
        objectId: '2',
